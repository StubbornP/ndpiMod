...
extern u_int32_t current_ndpi_memory, max_ndpi_memory,link_type;
...
struct ndpi_workflow * ndpi_workflow_init(const struct ndpi_workflow_prefs * prefs, pcap_t * _) {
...
void ndpi_workflow_process_packet (struct ndpi_workflow * workflow,
  ...
  const int datalink_type = link_type;
